...
input_file = './project-part/part-0.txt'
...
class Features:
    def __init__(self,project,token):
        ...
        if len(self.contributors) < 1000:
            return
        pulls = get_request_body('https://api.github.com/repos/' + self.project['full_name'] + '/pulls?state=closed&per_page=100&access_token='+self.token)
    ...
    def getFeatures(self, pull_request):
        ...
        def user_features(pull_request):
            ...
            if user['login'] in self.user_issues_merged.keys():
                ...
                issues_merged = get_request_body('https://api.github.com/search/issues?' + 'q=type:pr+author:{}+is:merged+archived:false&sort=created&per_page=100&order=asc&access_token={}'.format(user['login'],self.token))
                tmp = []
            ...
            if user['login'] in self.user_issues_all.keys():
                ...
                issues_all = get_request_body('https://api.github.com/search/issues?' + 'q=type:pr+author:{}+archived:false&sort=created&order=asc&per_page=100&access_token={}'.format(user['login'],self.token))
                tmp = []
                for i in issues_all:
                    tmp_dict = {}
                    tmp_dict['author_association'] = i['author_association']
            ...
            private_repos = 0
            public_repos = 0
            user_projects = get_request_body(user['repos_url']+'?per_page=100&access_token='+self.token) if not (user['login'] in self.user_project.keys()) else self.user_project[user['login']]
        ...
        def project_features(pull_request):
            ...
            '''
            Project_age = (time.mktime(end_time) - time.mktime(time.strptime(self.project['created_at'], "%Y-%m-%dT%H:%M:%SZ")))/3600/30
            Contributions = 0
