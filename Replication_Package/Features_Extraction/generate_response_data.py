      ...
      try:
          ...
          for project in project_list:
            if load_dict['url'].find(project) != -1:
                continue
            Wait_Time = {}
            load_dict['PR_Latency'] = (time.mktime(
                time.strptime(str(load_dict['Timeline'][1]['Created_At'])[2:-2], "%Y-%m-%dT%H:%M:%SZ")) - time.mktime(
                time.strptime(str(load_dict['Timeline'][0]['Created_At'])[2:-2], "%Y-%m-%dT%H:%M:%SZ"))) / 60
